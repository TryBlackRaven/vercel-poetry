...
{
  "builds": [
    {"src": "/app/main.py", "use": "@black-raven/vercel-poetry"}
  ],
  "routes": [
...
```
This **JSON configuration** specifies the builds and routes for a Vercel deployment, replacing the default `@vercel/python` builder with `@black-raven/vercel-poetry` for enhanced dependency management. 
...
This setup ensures that the main **Python** file is appropriately handled for deployment on Vercel using the custom `@black-raven/vercel-poetry` builder.
